...
export ProjectParams, InitializeModel, Solve_Model, VFI, GENERATE_K_GRID, ADJ_COST, IRR, OPTIMAL_H, OPERATING_PROFIT, OUTPUT, get_transition_matrix_young, stationary_distribution, moments, objective_function_SMM, new_objective_function_SMM
...
@with_kw struct ProjectParams
    ...
    #===Adjustment Cost & Policy===#
    gamma = 0.05          
    F = 0.01          
    ps = 0.80         
    tau::Float64 = 0.0  # Subsydium inwestycyjne (domyślnie 0.0 dla kompatybilności)
...
function InitializeModel(;
    alpha=0.3, v=0.6, r=0.04, delta=0.08, w=1.0, rho=0.9, sigma_eps=0.12, N_z=7, gamma=0.05,
    F=0.01, ps=0.80, tau=0.0, k_max=500, k_min=1e-4, N_A=500)
    beta = 1.0 / (1.0 + r)
    omega = range(0, 1, length=N_A)  
    ...
    return ProjectParams(
        ...
        gamma=gamma, F=F, ps=ps, tau=tau, k_min=k_min, k_max=k_max, N_A=N_A, omega=omega
    )
end
...
function IRR(model::ProjectParams, i)
    @unpack ps, tau = model
    if i >= 0
        return 1.0 - tau  # Cena zakupu z subsydium
    else
        return ps         # Cena sprzedaży bez zmian
    end
end
...
function OUTPUT(model::ProjectParams, k, z)  
    @unpack alpha, v = model
    h = OPTIMAL_H(model, k, z)
    return z * k^alpha * h^v
end
...
function Solve_Model(model::ProjectParams, V_old, min_error, max_iter)
    ...
    println("Całkowity czas obliczeń: ", round(total_time, digits=3), " s. Iteracji: $iter")
